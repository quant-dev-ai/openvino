...
Convolution1x1KernelEmitter::Convolution1x1KernelEmitter(
...
void Convolution1x1KernelEmitter::emit_impl(const std::vector<size_t>& in,
...
void Convolution1x1KernelEmitter::emit_isa(const std::vector<size_t> &in, const std::vector<size_t> &out) const {
    assert(in.size() == 3ul);
    //assert(out.size() == 1ul);
    if (out.size() != 1ul) {

...
static void TransformationUpToCPUSpecificOpSet(std::shared_ptr<ngraph::Function> nGraphFunc, const bool _enableLPT,
    ...
    ov::pass::VisualizeTree("svg/cpu.original.svg").run_on_model(nGraphFunc);
    ...
    //ov::pass::VisualizeTree("svg/cpu.fake_quantize_decomposition.svg").run_on_model(nGraphFunc);
    ...
    ov::pass::VisualizeTree("svg/cpu.lpt.svg").run_on_model(nGraphFunc);
    ...
    ov::pass::VisualizeTree("svg/cpu.post_lpt.svg").run_on_model(nGraphFunc);
    ...
    if (_enableSnippets && dnnl::impl::cpu::x64::mayiuse(dnnl::impl::cpu::x64::avx2)) {
        ...
        {
            ov::pass::VisualizeTree("svg/cpu.snippets.concatenate_constants1.svg").run_on_model(nGraphFunc);
            for (auto node : nGraphFunc->get_ops()) {
                auto subgraph = ngraph::as_type_ptr<ngraph::snippets::op::Subgraph>(node);
                if (subgraph != nullptr) {
                    ov::pass::VisualizeTree("svg/cpu.snippets.concatenate_constants1.body.svg").run_on_model(
                            subgraph->get_body());
            ...
            ov::pass::VisualizeTree("svg/cpu.snippets.concatenate_constants2.svg").run_on_model(nGraphFunc);
            for (auto node : nGraphFunc->get_ops()) {
                auto subgraph = ngraph::as_type_ptr<ngraph::snippets::op::Subgraph>(node);
                if (subgraph != nullptr) {
                    ov::pass::VisualizeTree("svg/cpu.snippets.concatenate_constants2.body.svg").run_on_model(
                            subgraph->get_body());
                    break;
                }
            }
    ...
    fqDecompositionManager.get_pass_config()->set_callback<ngraph::pass::FakeQuantizeDecomposition>([](const_node_ptr& node) -> bool {
            std::string errMsg;
            return node::FakeQuantize::isSupportedOperation(node, errMsg);
        });
